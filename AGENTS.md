...
This is a Decoupled Router template for creating contributed modules or themes. The project provides a complete development environment with CI configuration, testing setup, and deployment automation for publishing to Drupal.org.
...
- `decoupled_router.*` - Template extension files
- `DecoupledRouterService.php` - Main service class template
...
1. Run `php init.php` to customize template for Decoupled Router
2. Build environment: `make build` or `ahoy build`
3. Develop Decoupled Router code in `src/`
...
- SQLite database created in `/tmp/site_decoupled_router.sqlite`
